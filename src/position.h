...
public:
    Position() {
        ...
        for (int i = 0; i < PIECE_TYPE_NB; i++) byType[i] = 0ULL;
    ...
    void set_fen(const std::string& fen) {
        ...
        *this = Position();
        ...
        ss >> token;
        castlingRights = 0;
        if (token.find('K') != std::string::npos) castlingRights |= WHITE_OO;
        if (token.find('Q') != std::string::npos) castlingRights |= WHITE_OOO;
        if (token.find('k') != std::string::npos) castlingRights |= BLACK_OO;
        if (token.find('q') != std::string::npos) castlingRights |= BLACK_OOO;
