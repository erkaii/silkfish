...
namespace Search {
    ...
    inline Score search(Position& pos, Depth depth, Score alpha, Score beta) {
        ...
        if (tt_hit && tt_entry.depth >= depth) {
            if (tt_entry.bound == BOUND_EXACT) return tt_entry.score;
            if (tt_entry.bound == BOUND_LOWER) alpha = std::max(alpha, tt_entry.score);
            if (tt_entry.bound == BOUND_UPPER) beta = std::min(beta, tt_entry.score);
